...
Installs, updates, and picks between your AI coding CLIs — Claude Code,
Codex, Antigravity, Grok Build — shows quota/last-use for each and launches
whichever one you pick in automatic mode.

Everything below was verified directly against each CLI's real `--help`
output and live behavior on this machine (not copied blind from docs or
chat notes). Where something couldn't be confirmed, it's marked as such.

## Usage

`selectorai.py` is the primary version now (stdlib only, no dependencies).
`selectorai.sh` still works and stays in sync feature-wise, kept as a
dependency-free fallback for machines without Python.

```bash
./selectorai.py install              # install what's missing, update what's there
./selectorai.py status                # machine status + quota/reset info for each provider
./selectorai.py status --who          # same, plus who else is logged in (opt-in, see below)
./selectorai.py status --check-antigravity  # same, plus Antigravity's live usage % (opt-in, see below — avoids a Google login popup)
./selectorai.py check-antigravity on  # persist that on instead of typing the flag every run (see below)
./selectorai.py auth                  # log into every installed provider, one at a time, with a clean copy-pastable URL (see below)
./selectorai.py                       # interactive picker (see below), rank, launch the chosen one
./selectorai.py --refresh             # (or -r) force fresh network probe of quota, bypassing short-term cache
./selectorai.py "fix the bug in X"      # same, with an initial prompt
./selectorai.py --yolo                # Codex: full bypass (no sandbox) instead of the safer default
./selectorai.py --continue            # resume the chosen provider's most recent session instead of starting fresh
./selectorai.py -c --yolo "keep going"  # flags compose; order doesn't matter
./selectorai.py theme                 # pick the picker's visual theme — separate .tcss files, see below
```

Provider list lives in `PROVIDERS = [...]` at the top of `selectorai.py`
(`PROVIDERS=(...)` in the `.sh`). Adding a new one means filling in that list
plus the corresponding `PROVIDER_*` dicts / `provider_*` functions.
...
them. The Python side (`_run_textual_picker`) only owns *behavior*: widget
structure, key handling, what happens on Enter. It loads whichever theme is
selected via Textual's own `CSS_PATH`, so switching looks is a file swap,
never a code change — dropping a new `themes/<name>.tcss` in makes it
selectable automatically, no code to touch.
...
`_sysinfo_markup()` wraps just the `⚠ ...` tail in Rich markup
(`[yellow]...[/yellow]`) before it's set, only inside the Textual render
path. The plain `status`/fallback-menu output is untouched by this — no
markup tags leak into a plain terminal that doesn't interpret them.
...
## Language (`selectorai.py` only — see note below)

Three languages: **English (default)**, **Русский**, **Español**. Strings
live in `i18n/en.json` / `i18n/es.json` / `i18n/ru.json` next to the script
— not inline in the code — so adding a language or fixing a translation
means editing a JSON file, not touching Python. Each file is flat `key:
value` plus a `_name` field (the language's own name, shown in the `lang`
picker). `t(key, ...)` looks them up at runtime; provider flags
...
Antigravity's per-bucket resets — now shows a countdown next to the
absolute date, e.g. `resets in 2h 39m — Aug 17, 8:50am (...)` for something
hours away vs. `resets in 3d 21h — Aug 21, 4am (...)` for something days
out. The point is telling those two apart at a glance without doing the
subtraction yourself — a handful of rows, but they span wildly different
timescales (a 5h session window next to a ~monthly Codex reset).
...
`remoto desde <ip>`) and calls out other usernames or remote IPs explicitly.
...
`provider_status_grok()` doesn't attempt a live check at all — running
`grok -p "/usage"` wouldn't just fail to parse, it would spend real
...
tie some other way) keep `PROVIDERS`' own order (Claude, Codex, Antigravity,
Grok) among themselves, since Python's sort is stable. Applies to the
picker and both fallback menus alike.
...
on this machine, so their behavior is taken from `--help` text, not a live
run):
...
above, applied everywhere a provider might print a long auth URL, not just
...
session box-drawing itself across a corrupted-huge column count). Steps 1–2
below don't need the terminal's actual width to be right in the first place,
so the mutation was never load-bearing:
...
after this popped a real Google login window mid-session:
...
popup risk *every single launch*, confirmed in practice. `selectorai.py`
...
Doesn't affect this script either way: `launch_provider` never passes
...
- Not yet tested end-to-end over a real SSH session on this machine — the
  environment used to build this wasn't one, so `agy -p "/usage"` just hung
  trying (that's why its status check has a short 10s timeout instead of
  Claude's 20s, on top of now being opt-in — see `--check-antigravity` above).
...
- Resizing an actual terminal *window* wider also works, if that's easy on
  ...
  hard way while building this: it doesn't auto-revert, and if you later
  launch a full-screen TUI (`claude`, `codex`, `agy` themselves) in that same
...
- `guia-ai-limites-instalacion.md` in this folder is background material
  ...
  it first; treat it as inspiration, not documentation.

## Next / open items

- Test the SSH auth flow end-to-end.
- Optional cleanup: remove the old npm-based Codex if not already done
  (`sudo npm uninstall -g @openai/codex`) — already done on this machine.
